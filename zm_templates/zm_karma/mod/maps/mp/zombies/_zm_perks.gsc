...
third_person_weapon_upgrade( current_weapon, upgrade_weapon, packa_rollers, perk_machine, trigger )
{
    ...
    if ( isdefined( rel_entity ) )
    {
        ...
        // move the thundergun model down because its huge
        if ( upgrade_weapon == "thundergun_upgraded_zm" )
        {
            origin_offset -= ( 0, 0, 5.5 );
        }
    ...
    // spawn the unpapped variant because the model is all messed up
    if ( upgrade_weapon == "thundergun_upgraded_zm" )
    {
        trigger.worldgun = spawn_weapon_model( "thundergun_zm", undefined, origin_base + origin_offset, angles_base + angles_offset + vectorscale( ( 0, 1, 0 ), 90.0 ), weoptions );
    }
    else
    {
